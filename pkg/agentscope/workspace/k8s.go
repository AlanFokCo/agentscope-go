...
func NewK8sWorkspace(cfg K8sConfig) (*K8sWorkspace, error) {
	...
	if cfg.Token != "" {
		baseArgs = append(baseArgs, "--token="+cfg.Token)
	}
	if cfg.CACert != "" {
	...
	// Create Pod.
	if err := w.createPod(cfg.PodName, image, cfg.PVCName); err != nil {
...
func (w *K8sWorkspace) ListFiles(ctx context.Context, dir string) ([]FileInfo, error) {
	// Use find with a parseable format: type, size, filename.
...
func (w *K8sWorkspace) createPod(name, image, pvcName string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	// Check if pod already exists and is usable.
	args := w.baseKubectlArgsCopy()
	args = append(args, "get", "pod", name, "--ignore-not-found", "-o", "jsonpath={.status.phase}")
	result, err := w.runKubectl(ctx, args, nil)
	if err != nil {
		return err
	}
	...
	// Build pod JSON manifest.
	volumeMounts := ""
	volumes := ""
	if pvcName != "" {
		volumeMounts = `"volumeMounts":[{"name":"workspace-vol","mountPath":"/workspace"}],`
		volumes = fmt.Sprintf(`"volumes":[{"name":"workspace-vol","persistentVolumeClaim":{"claimName":%q}}],`, pvcName)
	}

	manifest := fmt.Sprintf(`{"apiVersion":"v1","kind":"Pod","metadata":{"name":%q,"namespace":%q},"spec":{%s"containers":[{"name":"workspace","image":%q,"command":["sleep","infinity"],%s"workingDir":"/workspace"}],"restartPolicy":"Never"}}`,
		name, w.namespace, volumes, image, volumeMounts)
